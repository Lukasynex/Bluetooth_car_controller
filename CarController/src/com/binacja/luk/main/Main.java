...
public class Main extends Activity implements SensorEventListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.car_controller_layout_mini);
...
//		setContentView(lay);
	...
	private boolean ClickedButton(float dx, float dy, int clicked) {
		...
		switch (clicked) {
		case 0: {// up
			if (dx < screenWidth / 4 && dy < screenHeight / 2)
				return true;
			else
				return false;
		}
		case 1: {// left
			if (dx > 3 * screenWidth / 4 && dy < screenHeight / 2)
				return true;
			else
				return false;
		}
		...
		case 4: {// sound
			if (dx > screenWidth / 4 && dx < screenWidth / 2
					&& dy < screenHeight / 2)
				return true;
			else
				return false;
		...
		case 6: {// acc++
			if (dx > screenWidth / 2 && dx < 3 * screenWidth / 4
					&& dy < screenHeight / 2)
				return true;
			else
				return false;
	...
	private void setButtons() {
		final RelativeLayout lay = (RelativeLayout) findViewById(R.id.Controller_Layout_mini);
		//final RelativeLayout layAceII = (RelativeLayout) findViewById(R.id.Controller_Layout);
		
		
		final TextView texxt = (TextView) findViewById(R.id.tvState);
